    ...
    class MttsSay(object):
        ...
        def reset_session(self, stop_audio=False):
            ...
            if had_active_generation_wait:
                try:
                    # SayBehavior only accepts ``dismiss`` while focused. The
                    # status/settings button may own focus, so use the
                    # unfocused variant to end the pending say interaction.
                    renpy.queue_event("dismiss_unfocused")
        ...
        def __call__(self, who, what, interact=True, *args, **kwargs):
            ...
            def wake_generation_wait(finished_task):
                ...
                if self._active_generation_wait_id != generation_wait_id:
                    return
                try:
                    # The spinner's say behavior may not be focused while an
                    # overlay button is hovered. ``dismiss_unfocused`` still
                    # reaches it and wakes the wait interaction.
                    renpy.queue_event("dismiss_unfocused")
                    renpy.restart_interaction()
                except Exception as e:
            ...
            while True:
                ...
                try:
                    interact_wait = True
                    self.call_old_say(who, self.build_generation_wait_text(is_extend, remaining_wait), interact_wait, args, kwargs)
                finally:
